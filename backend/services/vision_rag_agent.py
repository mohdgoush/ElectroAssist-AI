...
from backend.services.component_extractor import (
    extract_components
)
...
from backend.services.vision_parser import (
    parse_vision_response
)

def vision_rag_agent(
    image_path: str
):

    # Step 1
    vision_result = analyze_circuit_image(
        image_path
    )

    # Step 2
    vision_data = parse_vision_response(
        vision_result
    )

    components = vision_data[
        "components"
    ]
    # Step 3
    search_query = f"""
